...
Since then a second backend has moved in behind the same address, and it changes
what this service is for. See [What happened to the queue](#what-happened-to-the-queue)
at the end: the honest version is that the default path no longer goes through it.
...
The backend this was built for produces a cloned voice and is worth waiting for,
but it is slow, single-threaded and licensed for non-commercial use only, and the
company behind it shut down in 2024. A second engine now sits behind the same
endpoint. Measured on the same paragraph of text: 56.1 seconds for the original,
3.9 seconds for the new one. Roughly fourteen times faster, at the cost of a fixed
voice instead of a cloned one.

That engine is now the default, and it does not need the queue at all. It runs
beside it. So the scheduler described above, the thing this repository is mostly
about, is idle in normal operation.

I left it in, and the reason is not sentimental. The choice is a request header
and the default is one environment variable, so the slow engine is one line away
whenever a voice matters more than latency, and on that path the queue is still
the only thing standing between one slow renderer and every caller at once.
Deleting it would buy nothing and would have to be rebuilt the first time someone
sets `engine=xtts` on a batch job.

What I would not do is leave the README claiming a busy priority bus while the
default path walks past it. A service that quietly stops doing the thing it is
named after is how documentation starts lying.

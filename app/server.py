...
def waehle_engine(
    ...
    """Welches Backend diese Anfrage bedient.

    ★ EINE STIMMDATEI IST EINE ENGINE-WAHL. Piper kennt nur eingebaute
    Stimmnamen (`de_DE-thorsten_emotional-medium`); eine geklonte Stimme wie
    `stimme-de-frau.wav` kann es strukturell nicht erfuellen. Es faellt dann auf
    seine Standardstimme zurueck, weil eine unbekannte Stimme nicht in Stille
    enden darf (siehe piper.py) - und genau diese Rettung verdeckte den Fehler:
    seit dem 2026-09-06 bat `saganta-news-api` das Briefing taeglich um die
    geklonte Frauenstimme und bekam Thorsten. Sichtbar war es nur als
    `VoiceNotFoundError` im piper-Log, die Anfrage selbst meldete 200.

    Wer eine `.wav` nennt, hat XTTS gemeint. Ein ausdrueckliches `engine=`
    gewinnt weiterhin, damit die Vorgabe eine Entscheidung bleibt und kein Raten.
    """
    ausdruecklich = (kopfzeile or feld or "").strip().lower()
    if ausdruecklich:
        return ausdruecklich
    if speaker and speaker.strip().lower().endswith(".wav"):
        return "xtts"
    return vorgabe
...
async def handle_tts(request: web.Request) -> web.StreamResponse:
    ...
    # Sagt der Aufrufer nichts, gilt `engine_default` (seit 2026-09-06 piper,
    # Begruendung in config.py). Vorher fiel ein schweigender Aufrufer auf XTTS
    # und wartete damit rund vierzehnmal so lange, ohne es je gewaehlt zu haben.
    # Ausnahme: wer eine Stimmdatei nennt, meint XTTS (siehe waehle_engine).
    engine = waehle_engine(
        request.headers.get("X-TTS-Engine"),
        data.get("engine"),
        speaker,
        settings.engine_default,
    )
    if engine == "piper":
...
async def _on_startup(app: web.Application) -> None:
    ...
    app["piper"] = PiperBackend(
        settings.piper_host,
        settings.piper_port,
        basis_s=settings.piper_basis_s,
        s_je_zeichen=settings.piper_s_je_zeichen,
        max_s=settings.piper_max_s,
    )
    log.info(
